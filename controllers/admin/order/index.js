...
var ship = require('../../../services/ship.js')
var auth = require('../../../services/auth.js')

module.exports = function (router){
	router.post('/batch',auth.authUser,auth.authAdmin,batch_order)
	router.get('/view/:oid',auth.authUser,auth.authAdmin,order_detail)
	router.post('/create',auth.authUser,auth.authAdmin,create)
	router.get('/manage',auth.authUser,auth.authAdmin,order_manage)
}
...
function order_detail (req,res){
	var ep = new eventproxy()
		...
		models.Order.findById(req.params.oid)
		.then(function(order){
				ep.emit('order',order)
		})
}
        	...
        	for(var i =0;i<ids.length;i++){
        		(function(i){
        			models.Order.find({
        				...
        				order.state = state
        				order.save().then(function(order){
        					ship.rudeSync(order.id,function(err,order){
	...
	models.Order.findAll({
		...
		for(var i =0;i<orders.length;i++){
			(function(i){
                    ...
                    data.push("<a href=\"/admin/order/view/"+orders[i].id+"\">"+orders[i].id+"</a>")
                    ...
                    data.push("<a href=\"/admin/order/view/"+orders[i].id+"\" class=\"btn btn-xs default btn-editable\"><i class=\"fa fa-search\"><\/i> 详情<\/a>")
					ep.emit('datas',data)	
			})(i)
		}
